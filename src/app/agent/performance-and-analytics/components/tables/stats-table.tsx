import { demoCustomerManagementDetails } from "@/app/admin/admin.dto";
...
import { SalesChartData } from "@/app/admin/admin.dto";
import { demoLeadStatsDetails } from "@/app/agent/agent.dto";
...
const paginatedProperties = paginate(demoCustomerManagementDetails);

console.log(paginatedProperties);

const PerformanceAndAnalyticsStatsTable = () => {
...
<h2 className="text-xl my-6 font-semibold">Stats</h2>
				{/* chart */}
				<div className="h-[20rem] w-full my-3">
					<AreaChart
						data={demoLeadStatsDetails.StatsChartData}
						index="date"
						categories={["amount"]}
						curveType="monotone"
						colors={["black"]}
				...
				<div className="px-2 py-4 bg-white flex justify-between items-center border-t mt-4">
					<h2 className="text-lg font-semibold">Leads Daily</h2>
					...
					<table className="w-full border-collapse overflow-x-scroll">
						<thead className="text-left bg-gray-100 text-holmes-font-grey">
							<tr>
								...
								<th className="p-4">From Phone</th>
								<th className="p-4">From Message</th>
							</tr>
						</thead>
						<tbody className="bg-white">
							{demoLeadStatsDetails.dailyLeads.map((listing, index) => (
								<tr
									key={index}
								...
								>
									...
									<td className="p-4 min-w-[6rem]">{listing.from_phone}</td>
									<td className="p-4 min-w-[6rem]">{listing.from_message}</td>
						...
						<tbody className="bg-white">
							{propertiesShowing.data.map((listing, index) => (
								<tr
									...
									onClick={() => {
										router.push(
											`/admin/user-management/customer/${listing.id}`,
										);
									}}
								>
									<td className="p-4">
...
export default PerformanceAndAnalyticsStatsTable;
