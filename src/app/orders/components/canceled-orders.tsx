...
const paginatedOrders = paginate(demoOrders.canceled);
...
const CanceledOrdersTable = () => {
	...
	}>({ index: 0, data: paginatedOrders[0] });

	const [currentPage, setCurrentPage] = useState(1);

	const totalPages = paginatedOrders.length <= 3 ? paginatedOrders.length : 3;

	const handlePrevious = () => {
		if (currentPage > 1) {
			setOrdersShowing((prevState) => {
				const prevIndex = paginatedOrders.findIndex(
					(el, index) => prevState.index === index,
				);
				return {
					index: prevIndex - 1,
					data: paginatedOrders[prevIndex - 1],
				};
			});
