...
import { demoSavedListings } from "../saved-listings.dto";
...
const paginatedListings = paginate(demoSavedListings);
...
const SavedListingsTable = () => {
	...
	const [savedListingsShowing, setSavedListingsShowing] = useState<{
		index: number;
		data: {
			title: string;
			address: string;
	...
	}>({ index: 0, data: paginatedListings[0] });

	const [currentPage, setCurrentPage] = useState(1);

	const totalPages =
		paginatedListings.length <= 3 ? paginatedListings.length : 3;

	const handlePrevious = () => {
		if (currentPage > 1) {
			setSavedListingsShowing((prevState) => {
				const prevIndex = paginatedListings.findIndex(
					(el, index) => prevState.index === index,
				);
				return {
					index: prevIndex - 1,
					data: paginatedListings[prevIndex - 1],
				};
			});
			...
			<section className="TabletScreen:hidden MobileScreen:hidden">
				<div className="p-4 bg-white shadow-md rounded-md">
					...
					<table className="w-full border-collapse">
						...
						<tbody>
							{savedListingsShowing.data.map((listing, index) => (
								<tr
									key={index}
						...
						<tbody>
							{savedListingsShowing.data.map((listing, index) => (
								...
								>
									...
									<td className="p-4 min-w-[10rem]">{listing.date}</td>
									<td className="p-4 ">{listing.views}</td>
...
export default SavedListingsTable;
