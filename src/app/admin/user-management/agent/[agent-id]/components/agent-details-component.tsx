...
import { agentItemStructure } from "../../agent-management.dto";
...
const AgentDetailsComponent = () => {
	...
	const [propertiesShowing, setPropertiesShowing] = useState({
		index: 0,
		data: agentItemStructure.propertyStatus.properties[0],
	});
	const [currentPage, setCurrentPage] = useState(1);

	const totalPages =
		agentItemStructure.propertyStatus.properties.length <= 3
			? agentItemStructure.propertyStatus.properties.length
			: 3;
	...
	const handlePrevious = () => {
		if (currentPage > 1) {
			setPropertiesShowing((prevState) => {
				const prevIndex =
					agentItemStructure.propertyStatus.properties.findIndex(
						(el, index) => prevState.index === index,
					);
				return {
					index: prevIndex - 1,
					data: agentItemStructure.propertyStatus.properties[prevIndex - 1],
				};
			});
	...
	return (
		<section>
			<div className="p-4 bg-white rounded-md shadow-md mb-5">
				<div className="font-bold text-xl my-2">Agent Details</div>
				<div className="w-full h-[15rem]">
					<Image
						src={
							agentItemStructure.propertyStatus.properties[0].propertyPhotos[0]
						}
						alt="selected-agent-image"
				...
				<div className="p-4 shadow-md bg-white DesktopScreen:w-[68%] rounded-md">
					<div className="flex justify-between items-center mb-3">
						...
						<div className="p-4 flex items-center gap-4">
							<Image
								src={agentItemStructure.profilePicture}
								alt=""
								className="w-12 h-12 rounded-full object-cover"
							/>
							<div>
								<h3 className="font-[500] text-black">
									{agentItemStructure.name}
								</h3>
							</div>
						</div>
						...
						<div className="flex gap-4">
							...
							<div className="p-2 bg-red-100 hover:bg-red-200 rounded flex items-center min-w-10 justify-center">
								<Image src={redTrashIcon} alt="delete-agent" />
							</div>
						</div>
					</div>

					{/* agent location, num, email */}
					<div className="my-2">
						<div className="flex gap-3 items-center text-holmes-font-grey my-2">
							<Image alt="agent-location" src={greenLocationIcon} />
							<div>{agentItemStructure.location}</div>
						</div>
						<div className="flex gap-3 items-center text-holmes-font-grey my-2">
							<Image alt="agent-number" src={greenCallIcon} />
							<div>{agentItemStructure.contact}</div>
						</div>
						<div className="flex gap-3 items-center text-holmes-font-grey my-2">
							<Image alt="agent-email" src={greenEmailIcon} />
							<div>{agentItemStructure.email}</div>
						</div>
					</div>
					...
					<div className="my-3">
						<div className="my-2 text-xl font-bold">
							About {agentItemStructure.name}:
						</div>

						<p className="text-holmes-font-grey">
							{agentItemStructure.description}
						</p>
						<div className="my-3">
							<span className="text-xl mr-3 font-bold">Agency: </span>
							<span className="text-holmes-font-grey">
								{agentItemStructure.agency}{" "}
							</span>
						</div>
						<div className="my-3">
							<span className="text-xl mr-3 font-bold">Agent License: </span>
							<span className="text-holmes-font-grey">
								{agentItemStructure.agentLicense}{" "}
							</span>
						</div>
						<div className="my-3">
							<span className="text-xl mr-3 font-bold">Services Area: </span>
							<span className="text-holmes-font-grey">
								{agentItemStructure.servicesArea.map((el, index) => {
									if (index !== agentItemStructure.servicesArea.length - 1) {
										return `${el}, `;
									} else {
										return `${el}.`;
									}
									...
									<div className="flex gap-4">
										<div className="flex flex-col justify-between">
											...
											<div className="text-xl font-bold">
												{agentItemStructure.propertyStatus.totalListings}
											</div>
										</div>
								...
								<div className="bg-white border rounded-md p-6 DesktopScreen:my-0 my-2">
									...
									<div className="flex gap-4">
										<div className="flex flex-col justify-between">
											...
											<div className="text-xl font-bold">
												{agentItemStructure.propertyStatus.propertiesSold}
											</div>
										</div>
								...
								<div className="bg-white border rounded-md p-6 DesktopScreen:my-0 my-2">
									...
									<div className="flex gap-4">
										<div className="flex flex-col justify-between">
											...
											<div className="text-xl font-bold">
												{agentItemStructure.propertyStatus.propertiesRent}
											</div>
										</div>
						...
						<div className="my-3">
							...
							<div className="flex DesktopScreen:flex-row flex-col my-3 justify-between">
								{agentItemStructure.reviews.map((el, index) => {
									if (index > 1) {
										return;
									} else {
										return (
						...
						<div className="my-3">
							...
							<div className="flex justify-between flex-wrap my-2">
								{agentItemStructure.propertyFiles?.map((document, index) => {
									return (
										<div
											key={index}
