...
import { affliateItemStructure } from "../../affliate-management.dto";
...
const AffliateDetailsComponent = () => {
	const [propertiesShowing, setPropertiesShowing] = useState({
		index: 0,
		data: affliateItemStructure.propertyStatus.properties[0],
	});
	const [currentPage, setCurrentPage] = useState(1);

	const totalPages =
		affliateItemStructure.propertyStatus.properties.length <= 3
			? affliateItemStructure.propertyStatus.properties.length
			: 3;
	...
	const handlePrevious = () => {
		if (currentPage > 1) {
			setPropertiesShowing((prevState) => {
				const prevIndex =
					affliateItemStructure.propertyStatus.properties.findIndex(
						(el, index) => prevState.index === index,
					);
				return {
					index: prevIndex - 1,
					data: affliateItemStructure.propertyStatus.properties[prevIndex - 1],
				};
			});
	...
	return (
		<section>
			<div className="p-4 bg-white rounded-md shadow-md mb-5">
				<div className="font-bold text-xl my-2">Affliate Details</div>
				<div className="w-full h-[15rem]">
					<Image
						src={
							affliateItemStructure.propertyStatus.properties[0].propertyPhotos[0]
						}
						alt="selected-affliate-image"
				...
				<div className="p-4 shadow-md bg-white DesktopScreen:w-[68%] rounded-md">
					<div className="flex justify-between items-center mb-3">
						...
						<div className="p-4 flex items-center gap-4">
							<Image
								src={affliateItemStructure.profilePicture}
								alt=""
								className="w-12 h-12 rounded-full object-cover"
							/>
							<div>
								<h3 className="font-[500] text-black">
									{affliateItemStructure.name}
								</h3>
							</div>
						</div>
						...
						<div className="flex gap-4">
							...
							<div className="p-2 bg-red-100 hover:bg-red-200 rounded flex items-center min-w-10 justify-center">
								<Image src={redTrashIcon} alt="delete-affliate" />
							</div>
						</div>
					</div>

					{/* affliate location, num, email */}
					<div className="my-2">
						<div className="flex gap-3 items-center text-holmes-font-grey my-2">
							<Image alt="affliate-location" src={greenLocationIcon} />
							<div>{affliateItemStructure.location}</div>
						</div>
						<div className="flex gap-3 items-center text-holmes-font-grey my-2">
							<Image alt="affliate-number" src={greenCallIcon} />
							<div>{affliateItemStructure.contact}</div>
						</div>
						<div className="flex gap-3 items-center text-holmes-font-grey my-2">
							<Image alt="affliate-email" src={greenEmailIcon} />
							<div>{affliateItemStructure.email}</div>
						</div>
					</div>
					...
					<div className="my-3">
						<div className="my-2 text-xl font-bold">
							About {affliateItemStructure.name}:
						</div>

						<p className="text-holmes-font-grey">
							{affliateItemStructure.description}
						</p>
						<div className="my-3">
							<span className="text-xl mr-3 font-bold">Agency: </span>
							<span className="text-holmes-font-grey">
								{affliateItemStructure.agency}{" "}
							</span>
						</div>
						<div className="my-3">
							<span className="text-xl mr-3 font-bold">Affliate License: </span>
							<span className="text-holmes-font-grey">
								{affliateItemStructure.affliateLicense}{" "}
							</span>
						</div>
						<div className="my-3">
							<span className="text-xl mr-3 font-bold">Services Area: </span>
							<span className="text-holmes-font-grey">
								{affliateItemStructure.servicesArea.map((el, index) => {
									if (index !== affliateItemStructure.servicesArea.length - 1) {
										return `${el}, `;
									} else {
										return `${el}.`;
									}
									...
									<div className="flex gap-4">
										<div className="flex flex-col justify-between">
											...
											<div className="text-xl font-bold">
												{affliateItemStructure.propertyStatus.totalListings}
											</div>
										</div>
								...
								<div className="bg-white border rounded-md p-6 DesktopScreen:my-0 my-2">
									...
									<div className="flex gap-4">
										<div className="flex flex-col justify-between">
											...
											<div className="text-xl font-bold">
												{affliateItemStructure.propertyStatus.propertiesSold}
											</div>
										</div>
								...
								<div className="bg-white border rounded-md p-6 DesktopScreen:my-0 my-2">
									...
									<div className="flex gap-4">
										<div className="flex flex-col justify-between">
											...
											<div className="text-xl font-bold">
												{affliateItemStructure.propertyStatus.propertiesRent}
											</div>
										</div>
						...
						<div className="my-3">
							...
							<div className="flex DesktopScreen:flex-row flex-col my-3 justify-between">
								{affliateItemStructure.reviews.map((el, index) => {
									if (index > 1) {
										return;
									} else {
										return (
						...
						<div className="my-3">
							...
							<div className="flex justify-between flex-wrap my-2">
								{affliateItemStructure.propertyFiles?.map((document, index) => {
									return (
										<div
											key={index}
...
export default AffliateDetailsComponent;
