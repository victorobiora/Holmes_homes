...
//first we split the array to contain only scheduled notifications
    const finalScheduledNotificationsArray = demoNotificationSettingsDetails.filter(el => el.status === 'Scheduled')

//then we paginate to show only 5 per row
const paginatedNotifications = paginate(finalScheduledNotificationsArray);
